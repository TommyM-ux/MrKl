document.addEventListener('DOMContentLoaded', () => {
  ...
  function generate() {
    ...
    const bodyLenCm = Math.round(finished * 0.38); // odhadovaná délka

    const armDepthCm = Math.round(finished * armRatio * 10) / 10;
    const armRows    = EVEN(Math.round(armDepthCm * rowsPerCm));

    const armholeBO    = 3; // základní uzavření v podpaží
    const armholeDrop  = 12; // celkový úbytek ok pro průramek
    ...
    // AKTUALIZACE POMOCNÝCH POLÍ VE FORMULÁŘI
    if ($('finishedOut')) $('finishedOut').textContent = finished;
    if ($('finishedExplain')) $('finishedExplain').textContent = `(${chest} cm tělo + ${ease} cm volnost)`;

    // GENEROVÁNÍ TEXTOVÉHO NÁVODU
    const out = $('out');
    if (out) {
      out.innerHTML = `
